...
export default function Post() {
  ...
  const submitPost = async (e) => {
    ...
    //Run checks for Buzz Description
    if (!post.description){
      toast.error("Buzz Description Field Empty :)", {
    ...
    if (post.description.length > 300){
      toast.error("Buzz Description too long :)", {
        position: toast.POSITION.TOP_CENTER,
        autoClose:1500,
      })
    ...
    setPost({ description: ""});
    toast.success('Post has been made XD', {position: toast.POSITION.TOP_CENTER,
      autoClose: 1500,})
    return route.push("/");
    }
  };
